...
extern uint8_t Pick_up_stop; // 检查是否被拿起标志位
extern int Middle_angle;     // 机械中值默认为0
extern uint8_t Lidar_Detect;
extern uint8_t Mode;                                                                          // 模式选择，默认是普通的控制模式
extern uint8_t PS2_ON_Flag;                                                                   // 默认所有方式不控制
...
extern uint16_t Flag_front, Flag_back, Flag_Left, Flag_Right, Flag_velocity, Target_Velocity; // 蓝牙遥控相关的变量
extern uint8_t Flag_Stop, Flag_Show;                                                          // 停止标志位和 显示标志位 默认停止 显示打开
extern int Voltage;                                                                           // 电池电压采样相关的变量
extern float Angle_Balance, Gyro_Balance, Gyro_Turn;                                          // 平衡倾角 平衡陀螺仪 转向陀螺仪
extern int Temperature;
extern uint32_t Distance;               // 雷达测距
extern uint16_t determine;              // 确定走直线的距离值
...
extern uint8_t CCD_Zhongzhi, CCD_Yuzhi;             // 线性CCD
