...
void data_process(void) //数据处理
{
	...
	uint8_t i,m,n;
	uint32_t distance_sum[8]={0};//2个点的距离和的数组
	float start_angle = (((uint16_t)Pack_Data.start_angle_h<<8)+Pack_Data.start_angle_l)/100.0;//计算16个点的开始角度
	float end_angle = (((uint16_t)Pack_Data.end_angle_h<<8)+Pack_Data.end_angle_l)/100.0;//计算16个点的结束角度
	...
	for(i=0;i<16;i++)
	{
		...
		{
			case 0:case 1:
				distance_sum[0] +=((uint16_t)Pack_Data.point[i].distance_h<<8)+Pack_Data.point[i].distance_l;//0~1点的距离和
			  break;
			case 2:case 3:
				distance_sum[1] +=((uint16_t)Pack_Data.point[i].distance_h<<8)+Pack_Data.point[i].distance_l;//2~3点的距离和
			  break;
			case 4:case 5:
				distance_sum[2] +=((uint16_t)Pack_Data.point[i].distance_h<<8)+Pack_Data.point[i].distance_l;//4~5点的距离和
			  break;
			case 6:case 7:
				distance_sum[3] +=((uint16_t)Pack_Data.point[i].distance_h<<8)+Pack_Data.point[i].distance_l;//6~7点的距离和
			  break;
			case 8:case 9:
				distance_sum[4] +=((uint16_t)Pack_Data.point[i].distance_h<<8)+Pack_Data.point[i].distance_l;//8~9点的距离和
			  break;
			case 10:case 11:
				distance_sum[5] +=((uint16_t)Pack_Data.point[i].distance_h<<8)+Pack_Data.point[i].distance_l;//10~11点的距离和
			  break;
			case 12:case 13:
				distance_sum[6] +=((uint16_t)Pack_Data.point[i].distance_h<<8)+Pack_Data.point[i].distance_l;//12~13点的距离和
			  break;
			case 14:case 15:
				distance_sum[7] +=((uint16_t)Pack_Data.point[i].distance_h<<8)+Pack_Data.point[i].distance_l;//14~15点的距离和
			  break;
			default:break;
